...
export async function getOrderByIdOrNumber(idOrNumber) {
    ...
    let row = null;
    const byId = await query('SELECT * FROM orders WHERE id=$1', [idOrNumber]);
    if (byId.rows.length)
        row = byId.rows[0];
    if (!row) {
        const byNum = await query('SELECT * FROM orders WHERE order_number=$1', [idOrNumber]);
        if (byNum.rows.length)
            row = byNum.rows[0];
    }
    if (!row)
        return null;
    const items = await query('SELECT * FROM order_items WHERE order_id=$1', [row.id]);
...
export async function createOrder(data) {
    ...
    if (!orderNumber) {
        const c = await query('SELECT COUNT(*)::int AS count FROM orders');
        const next = (Number(c.rows[0]?.count || 0) + 1);
        orderNumber = `ORD${String(next).padStart(6, '0')}`;
    }
    const { userId, restaurantId, restaurantName, total, status = 'pending', paymentMethod = 'cash', paymentStatus = 'pending', deliveryAddress, customerName, customerPhone, customerEmail, estimatedTime, notes } = data;
    ...
    for (const it of items) {
        await query(`INSERT INTO order_items (id, order_id, item_id, name, price, quantity, image)
       VALUES ($1,$2,$3,$4,$5,$6,$7)`, [genId('oit'), id, it.itemId, it.name, it.price, it.quantity, it.image ?? null]);
    }
    const inserted = await query('SELECT * FROM orders WHERE id=$1', [id]);
    const rowsItems = await query('SELECT * FROM order_items WHERE order_id=$1', [id]);
    return toApiOrder(inserted.rows[0], rowsItems.rows);
}
export async function updateOrderByIdOrNumber(idOrNumber, update) {
    // Resolve target id
    const target = await query('SELECT * FROM orders WHERE id=$1 OR order_number=$1', [idOrNumber]);
...
export async function dashboardStats() {
    const totalOrdersQ = await query('SELECT COUNT(1)::int AS count FROM orders');
    const revenueQ = await query('SELECT COALESCE(SUM(total),0) AS total_revenue FROM orders');
    const pendingQ = await query("SELECT COUNT(1)::int AS count FROM orders WHERE status='pending'");
    const completedQ = await query("SELECT COUNT(1)::int AS count FROM orders WHERE status='delivered'");
    const recentQ = await query('SELECT * FROM orders ORDER BY created_at DESC LIMIT 5');
    ...
    const topItemsQ = await query(`SELECT oi.item_id as id, oi.name, SUM(oi.quantity) as quantity,
...
function toApiOrder(row, itemsRows) {
    return {
        id: row.id,
