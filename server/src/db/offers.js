...
export async function listOffers(filter) {
    ...
    const { rows } = await query(`SELECT * FROM offers ${where} ORDER BY created_at DESC`);
...
export async function createOffer(data) {
    ...
    await query(`INSERT INTO offers (id, title, description, discount_type, discount_value, min_order_amount, max_discount_amount, valid_from, valid_until, is_active, applicable_on, item_ids)
...
export async function updateOffer(id, update) {
    const fields = [];
    const params = [];
    let idx = 1;
    const map = {
