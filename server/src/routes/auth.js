        ...
        if (reqEmail === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
            try {
                ...
                if (!adminUser) {
                    ...
                    adminUser = await createUser({ name: 'Admin User', email: ADMIN_EMAIL, password: ADMIN_PASSWORD, role: 'admin' });
                }
                else if (adminUser.role !== 'admin' && adminUser.role !== 'super-admin') {
                    // Promote existing user to admin
                    const updated = await updateUser(adminUser.id, { role: 'admin' });
                ...
                const token = jwt.sign({ userId: 'admin-bypass', email: ADMIN_EMAIL, role: 'admin' }, JWT_SECRET, { expiresIn: '7d' });
...
router.post('/otp/verify', async (req, res) => {
    ...
    if (!user) {
        ...
        user = { id: created.id, name: created.name, email: created.email, role: created.role, phone };
    }
    // Issue JWT
...
router.post('/clerk-verify', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const allowBypass = (process.env.ALLOW_AUTH_BYPASS ?? (process.env.NODE_ENV !== 'production' ? 'true' : 'false'));
        ...
        const allowAllAdminDev = (process.env.ALLOW_ALL_AS_ADMIN_IN_DEV ?? (process.env.NODE_ENV !== 'production' ? 'true' : 'false')).toLowerCase() === 'true';
        ...
        // 3. Issue backend JWT
        const yourJwt = jwt.sign({ userId: user.id, email: user.email, clerkId: user.clerkId, role: user.role }, process.env.JWT_SECRET || 'your-secret-key', { expiresIn: '7d' });
